...
void intmath_div(uint64_t* io_lo, uint64_t* o_hi, uint64_t n, uint8_t isSigned, uint8_t bits) {
	if (bits == 8)
		divBB(io_lo, o_hi, n, isSigned);
	else if (bits == 16)
		divWW(io_lo, o_hi, n, isSigned);
	else if (bits == 32)
		divDD(io_lo, o_hi, n, isSigned);
	else
		divQQ(io_lo, o_hi, n, isSigned);
}
